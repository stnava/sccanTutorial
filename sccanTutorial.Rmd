...
batt<-sparseDecom2( inmatrix=list(rptrainimg,rcogmat), its=15, 
  sparseness=c(0.01, -0.1), inmask=c(mask,NA), nvecs=10, cthresh=c(100,0),
  smooth=0.5, ell1=-10, mycoption=1 )
...
for ( bestpred in 1:nrow(ccamat)) {
  ...
  cogpred<-( cogmat %*% data.matrix( batt$eig2 ) )[,bestpred]
  ...
  for ( x in which.min(p.adjust(mdlinterp$beta.pval)) )  {
    ...
    rid3<-rotate3d( id , -pi/2, 0, 0, 1 )
    par3d(userMatrix = id ) 
    dd<-make3ViewPNG(  rid, id, rid2, paste(rootdir,'/figures/battery',bestpred,sep='') )
    par3d(userMatrix = id ) 
    }
    cat("\n")
  }
