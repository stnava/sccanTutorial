...
mdl1<-lm( trainCorrs ~ stats::poly(sparseness,1), data=mydf ) 
mdl2<-lm( testCorrs ~ stats::poly(sparseness,1) , data=mydf ) 
visreg(mdl1)
visreg(mdl2)
```
...
langresult<-sparseDecom2( inmatrix=list(ptrainimg,langmat), its=12, mycoption=1,
  sparseness=c(sp,-0.3), inmask=c(mask,NA),nvecs=length(labels), cthresh=c(5,0), 
  initializationList=ccainit$initlist, priorWeight=0.3, smooth=0.5, ell1=10 )
...
myform<-as.formula( "Variate001~GM1+GM2+GM3+GM4" )
mdltrain<-lm( myform, data=mydf )
langpred<-ptestimg %*% t(ccamat)
colnames(langpred)<-paste("GM",c(1:ncol(langpred)),sep='')
cogpred<-langmat2 %*% data.matrix( langresult$eig2 )
mydf<-data.frame( cogpred, langpred )
print(cor.test( mydf$Variate001 ,predict(mdltrain,newdata=mydf)))
for ( i in 1:length(labels) )
...
for ( x in langresult$eig1 ) {
  ImageMath(3,x,"abs",x)
  ImageMath(3,x,"Normalize",x)
  print(paste(max(x),min(x)))
}
mycolors<-c("red","green","blue","yellow")
langfn<-paste(rootdir,'/figures/langSCCANRegression.jpg',sep='')
plotANTsImage( mask, functional=langresult$eig1, threshold='0.25x1',
  slices="12x50x1",color=mycolors,outname=ofn )
```
