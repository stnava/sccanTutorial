...
for ( sp in paramsearch ) {
  ...
  ageresult<-sparseDecom2( inmatrix=list(ptrainimg,agemat), its=its,
    sparseness=c(sp,0.9), inmask=c(mask,NA),nvecs=2, mycoption=1,
    cthresh=c(250,0), ell1=0.1,
    smooth=0.5, verbose=verb ) # initializationList = initdf$initlist,
...
langresult<-sparseDecom2( inmatrix=list(ptrainimg,langmat), its=its,
  mycoption=1,
  sparseness=c(sp,-0.5), inmask=c(mask,NA),nvecs=length(labels), cthresh=c(100,0), verbose=1,
  initializationList=ccainit$initlist, priorWeight=pw/100, smooth=0.0,
  perms=0 )
...
batt2<-sparseDecom2( inmatrix=list(rptrainimg,rcogmat), its=its,
  ...
  cthresh=c(100,0), smooth=0.0, mycoption=0 )
ccamat = batt2$eig1
