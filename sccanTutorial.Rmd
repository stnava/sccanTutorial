...
langresult<-sparseDecom2( inmatrix=list(ptrainimg,langmat), its=15, mycoption=1,
  sparseness=c(sp,-0.3), inmask=c(mask,NA),nvecs=length(labels), cthresh=c(150,0),
...
brain<-renderSurfaceFunction( surfimg =list( aalimg ) , alphasurf=0.1 ,
  funcimg=langresult$eig1, smoothsval=1.5, smoothfval=1.0, mycol=mycolors )
id<-par3d("userMatrix")
...
```{r covar1, fig.width=8, fig.height=4, echo=TRUE, messages=FALSE }
# 1. control for age and mmse after the dimensionality reduction
langresult<-sparseDecom2( inmatrix=list(ptrainimg,langmat), its=15, mycoption=1,
  sparseness=c(sp,-0.3), inmask=c(mask,NA),nvecs=length(labels), cthresh=c(50,0),
  initializationList=ccainit$initlist, priorWeight=pw/100, smooth=0.0, ell1=-10 )
ccamat<-imageListToMatrix( langresult$eig1, mask )
langpred<-ptrainimg %*% t(ccamat)
colnames(langpred)<-paste("GM",c(1:ncol(langpred)),sep='')
...
bestpred<-which.max(abs(diag(cor(langpred,cogpred))))
mydf<-data.frame( cogpred, langpred )
myform<-as.formula( paste("Variate00",bestpred-1,"~GM1+GM2+GM3+GM4+ptraincog$mmse+ptraincog$age",sep='') )
mdltrain<-lm( myform, data=mydf )
print(summary(mdltrain))
```

Now, the second option.
```{r covar2, fig.width=8, fig.height=4, echo=TRUE, messages=FALSE }
...
langresult<-sparseDecom2( inmatrix=list(rptrainimg,rlangmat), its=15, mycoption=1,
  ...
  initializationList=ccainit$initlist, priorWeight=pw/100, smooth=0.0, ell1=-10 )
ccamat<-imageListToMatrix( langresult$eig1, mask )
langpred<-rptrainimg %*% t(ccamat)
colnames(langpred)<-paste("GM",c(1:ncol(langpred)),sep='')
cogpred<-rlangmat %*% data.matrix( langresult$eig2 )
bestpred<-which.max(abs(diag(cor(langpred,cogpred))))
mydf<-data.frame( cogpred, langpred )
myform<-as.formula( paste("Variate00",bestpred-1,"~GM1+GM2+GM3+GM4",sep='') )
mdltrain<-lm( myform, data=mydf )
print(summary(mdltrain))
# note: test data is not residualized
langpred<-ptestimg %*% t(ccamat)
colnames(langpred)<-paste("GM",c(1:ncol(langpred)),sep='')
cogpred<-langmat2 %*% data.matrix( langresult$eig2 )
mydf<-data.frame( cogpred, langpred )
...
myform<-as.formula( paste("Variate00",bestpred-1,
  "~GM1+GM2+GM3+GM4+ptestcog$mmse+ptestcog$age",sep='') )
mdltest<-lm( myform, data=mydf )
print( summary( mdltest ) )
```
...
```{r fullpred, fig.width=8, fig.height=4, echo=TRUE, messages=FALSE }
nv<-11
nfn<-rep("",nv)
...
batt<-sparseDecom2( inmatrix=list(rptrainimg,rcogmat), its=15,
  sparseness=c(0.01, -0.1), inmask=c(mask,NA), nvecs=nv, cthresh=c(100,0),
  smooth=0.5, ell1=-10, mycoption=0 )
...
```{r fullreport, fig.width=8, fig.height=4, echo=TRUE, messages=FALSE }
render<-TRUE
for ( bestpred in 1:nrow(ccamat)) {
  ...
  cat( colnames(ptraincog)[ abs(batt$eig2[,bestpred]) > 0 ])
  cat("\nwith weights\n")
  ...
  for ( x in which.min(p.adjust(mdlinterp$beta.pval)) )  {
    cat((reportAnatomy( list( batt$eig1[[x]]) , mask , 0.5 )))
    if ( render ) {
...
predictedBattery<-data.frame( vox=ptrainimg %*% t(ccamat) %*% t(batt$eig2) )
print(diag(cor(cogmat,predictedBattery)))
predictedBattery<-data.frame( vox=ptestimg %*% t(ccamat) %*% t(batt$eig2) )
print(diag(cor(data.matrix(ptestcog),predictedBattery)))
qv<-rep(NA,ncol(ptestcog) )
for ( i in 1:ncol(ptestcog) ) {
 print(colnames(ptestcog)[i])
...
using SCCAN multivariate mapping: `r print( colnames(ptestcog)[ qv <= 0.05 ] )`.
